...
create or replace function oir.at_risk_201710_pred_table_build(output_schema text, daily_table_waited_days int) returns void as
$BODY$
...
(
    ...
    where
        ...
        academic_period in ('201630', '201710') 
    group by 1
...
FOR i in 1..array_upper(all_tables, 1) LOOP
	...
	IF all_tables[i] not in (
		...
		where 
            ...
            column_name !~ E'hs\\_state\\_(?!in|il)[a-z]+' --exclude hs_state_... columns in high_school table, except _in and _il states
		into feature_list;
...
EXECUTE $$drop table if exists $$ || output_schema || $$.$$ || $$at_risk_201710_pred_table_$$ || current_date_str || $$ cascade$$;
EXECUTE $$create table $$ || output_schema || $$.$$ || $$at_risk_201710_pred_table_$$ || current_date_str || $$ as
(
...
--select oir.at_risk_201710_pred_table_build('oir', 30);
