...
--------------------------------------------------------------------------------------------------------------------------
-- Compared to 2016 model, we added or need to add 1) admitted college 2) fall/spring indicator for admitted semester 3) SIF features
-- Need to experiment with tree based regression to generate predicted GPA instead of probablity score
-- e.g. http://scikit-learn.org/stable/auto_examples/tree/plot_tree_regression.html
...
FOR i in 1..array_upper(all_tables, 1) LOOP
	...
	IF all_tables[i] not in (
		...
		where 
            ...
            column_name !~ E'profile\\_(?!academic\\_period).+' and  --only include profile_academic_period in feats_from_profile_table, for filtering purposes; it should be excluded for modeling
...
(
    ...
    where profile_academic_period::int < 201710
) distributed by (person_uid)$$;
...
(
    select * from at_risk_temp_table
    where profile_academic_period in ('201710')
) distributed by (person_uid)$$;
