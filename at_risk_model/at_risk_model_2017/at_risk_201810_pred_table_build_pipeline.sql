...
--                                                 April 2016                                                           --
--                                      Prepared for model refresh in 2017 spring                                       --
--------------------------------------------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------------------------------------------
...
create or replace function oir.at_risk_201810_pred_table_build(output_schema text, daily_table_waited_days int) returns void as
$BODY$
...
auto_refresh_tables = array[
    'sis.applications'   --only need this table to extract applied academic_period for filtering purposes
    ];

all_tables = daily_refresh_tables || auto_refresh_tables;
...
(
    ...
    from 
        sis.applications
    where
        latest_decision in ('AA','AN') and
        campus = 'PWL' and
        admissions_population in ('B', 'SB') and
        student_level in ('U', 'UG') and
        curriculum_priority_number = 1 and
        academic_period in ('201730', '201810') 
    group by 1
...
FOR i in 1..array_upper(all_tables, 1) LOOP
	...
	IF all_tables[i] not in (
		...
		where 
            ...
            column_name !~ E'app\\_race.+'   --exclude app_race... in application_demos table, use reporting_ethnicity_... from feats_from_person_detail_table instead
		into feature_list;
...
EXECUTE $$drop table if exists $$ || output_schema || $$.$$ || $$at_risk_201810_pred_table_$$ || current_date_str || $$ cascade$$;
EXECUTE $$create table $$ || output_schema || $$.$$ || $$at_risk_201810_pred_table_$$ || current_date_str || $$ as
(
...
--select oir.at_risk_201810_pred_table_build('oir', 30);
