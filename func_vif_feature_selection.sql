...
--                          Srivatsan Ramanujam<sramanujam@pivotal.io>, Sep-2015                                             --
-------------------------------------------------------------------------------------------------------------------------------


-----------------------------------------------------------------------------------------------------
-- 1) Define UDF to compute ViF scores
-- Note: Please ensure your input table has NULL values imputed with mean/median or so
...
drop function if exists oir.filter_array_at_index(text[], int);
create or replace function oir.filter_array_at_index(
    arr text[],
    indx int
)
returns text[]
...
drop function if exists oir.filter_array_at_index(int[], indx int);
create or replace function oir.filter_array_at_index(
    arr int[],
    indx int
)
...
drop function if exists oir.filter_array_at_index(float8[], indx int);
create or replace function oir.filter_array_at_index(
    arr float8[],
    indx int
)
...
drop function if exists oir.feature_selection_w_vif(
    text, 
    text, 
    text, 
    text, 
...
create function oir.feature_selection_w_vif(
...
create or replace function oir.feature_selection_w_vif(
    input_table text,
    id_column text,
        ...
        for f in range(len(fnames)):
            ...
            sql = """
                ...
                (
                    select 
                        id,
                        oir.filter_array_at_index(features, {indx}) as features,
        ...
        if max_vif < vif_threshold:
            ...
            vif_fselect_table_sql = """
                ...
                (
                    select 
                        id,
                        oir.filter_array_at_index(feature_names, {feat_indx}) as feature_names,
                        oir.filter_array_at_index(features, {feat_indx}) as features
                    from 
                        vif_fselection_tbl_{UUID}
                ) distributed by (id);
            """.format(
...
select oir.feature_selection_w_vif(
    'input_table_for_modeling_round3_all_num', -- Input table
    'rr', -- input schema
    ...
    'sr.vif_results_table' -- Results table
);

--b) If your table is of the form ||id | feature_names text[] | feature_vals float8[]||
select oir.feature_selection_w_vif(
    'sr.input_table_for_modeling_round3_all_num_imputed', -- Input table
    'person_uid', -- ID column
